...
export type OrderSkuItem = {
  id: string // 对应 item.id
  spuId: string
  goodsName: string // 对应 item.goodsName
  skuAttrs: string // 对应 item.skuAttrs
  quantity: number
  payPrice: number
  picture: string // 对应 item.picture
}
...
//售后类型
export type AfterSaleParams = {
  orderId: string
  type: number // 1取消/改单 2申诉退款 3投诉差评 4发票申请
  reason: string
  imgUrls?: string // 多图逗号分隔
}
